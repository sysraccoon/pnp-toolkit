    ...
    def rotate(self, rotate_times: int = 1) -> "PaperSpec":
        ...
        for _ in range(rotate_times):
            rotated_paper.margins = Offset(
                rotated_paper.margins.left_mm,
                rotated_paper.margins.top_mm,
                rotated_paper.margins.right_mm,
                rotated_paper.margins.bottom_mm,
            )
...
class CombinedLayout:
    ...
    def __post_init__(self):
        ...
        if self.search_optimal_paper_orientation:
            self.paper = self._search_effective_orientation(
                self.paper, self._image_size
            )
        self._paper_work_area = self.paper.work_area()
        if not self.image_nup:
            self.image_nup = self._calculate_effective_layout(
                self.paper.work_area(), self._image_size
            )
        self._image_per_page = self.image_nup[0] * self.image_nup[1]
        self._layout_area = Size(
            self._image_size.width_mm * self.image_nup[0],
            self._image_size.height_mm * self.image_nup[1],
        )
    ...
    def coordinates_to_position(self, x, y, mirror=False):
        ...
        offset_x_for_center = (
            self._paper_work_area.width_mm - self._layout_area.width_mm
        ) / 2 + self.paper.margins.left_mm
        offset_y_for_center = (
            self._paper_work_area.height_mm - self._layout_area.height_mm
        ) / 2 + self.paper.margins.top_mm
    ...
    def _search_effective_orientation(paper: PaperSpec, image_size: Size) -> PaperSpec:
        ...
        landscape_image_count = CombinedLayout._calculate_image_count(
            landscape_paper, image_size
        )
    ...
    @staticmethod
    def _calculate_effective_layout(
        work_area: Size, image_size: Size
    ) -> Tuple[int, int]:
