...
def generate_pdf(pdf_path, images_with_copy_count, combined_layout, background_pattern=None, signature=None):
    try:
            ...
            for _ in range(count):
                if placed_images_to_page >= combined_layout._image_per_page:
                    ...
                    if background_pattern:
                        generate_background_page(canvas, combined_layout, background_pattern, placed_images_to_page)
                    placed_images_to_page = 0
                    page_number += 1
                place_image_by_index(canvas, image, combined_layout, placed_images_to_page)
...
def place_label(canvas, combined_layout, label):
    ...
    pos_x, pos_y = combined_layout.coordinates_to_position(cor_x, cor_y, combined_layout.image_layout.mirror)
    LABEL_OFFSET_MM = 5
...
def generate_background_page(canvas, combined_layout, background_pattern, background_count):
    ADDITIONAL_BACKGROUND_BORDER_WIDTH = 3
    for index in range(background_count):
        if combined_layout.image_layout.background_border:
            left_offset, top_offset = index_to_mm_coordinates(combined_layout, index, True)
            ...
            canvas.rect(left_offset * mm, top_offset *mm,
                        width=width * mm, height=height *mm,
                        fill=1)

    for index in range(background_count):
        place_image_by_index(canvas, background_pattern, combined_layout, index, mirror=True)
    canvas.showPage()
...
def _get_left_top_offset(combined_layout, x, y) -> Tuple[float, float]:
    left_offset = x
    top_offset = combined_layout.paper.size.height_mm - y - combined_layout._image_size.height_mm
...
def _draw_image(canvas, image, left, top, image_size: Size):
    canvas.drawImage(
        image, left * mm, top *mm, width=image_size.width_mm * mm, height=image_size.height_mm * mm, mask="auto"
    )
