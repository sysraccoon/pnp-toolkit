...
@pytest.mark.parametrize("measure_str,expected_value,expected_unit", [
    ("10.2inch", 10.2, "inch"), 
    (" 5.111 mm  ", 5.111, "mm"),
    ("5 in", 5, "in"),
    ("7", 7.0, None),
...
@pytest.mark.parametrize("base_value,base_unit,expected_value,expected_unit", [
    (10.2, "inch", 10.2, "inch"), 
    (10.0, "inch", 254.0, "mm"),
    (1.5, "m", 1500, "mm"),
])
def test_convert_raw_measure_to_unit(base_value, base_unit, expected_value, expected_unit):
    actual_value = measures._convert_raw_measure_to_unit(base_value, base_unit, expected_unit)
    _assert_float(actual_value, expected_value)


@pytest.mark.parametrize("measure_pair,expected_pair", [
